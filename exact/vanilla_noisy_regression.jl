...
fX, noiseX, yX = rand(rng, [f(X_prior, 1e-9), ε(X_prior, 1e-9), y(X_prior, 1e-9)]);
...
f′Xp, ε′Xp = rand(rng, [f′(Xplot, 1e-9), ε′(Xplot, 1e-9)], S);

# Get posterior mean and marginals f′ and y′ and write them for plotting.
ms_f = marginals(f′(Xplot));
ms_y = marginals(y′(Xplot));

μ′f, σ′f = mean.(ms_f), std.(ms_f);
σ′y = std.(ms_y);
